...
static void place(void *bp, size_t asize) {
    ...
    /* if fits, don't do anything; if it doesn't and its size is at least asize + 16, change the header to asize and allocated; add a footer at a correct distance from the header, set it to asize and allocated; the next block is the header of the remaining free stuff; set it to previous bp size - asize and unallocated; go to where its footer should be, set it to the same.
